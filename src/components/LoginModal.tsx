...
import { Glasses, Shield, User, Lock, Mail, AlertCircle, ArrowRight, UserCheck, X } from "lucide-react";
      ...
      <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-slate-200 text-slate-800 relative">
        {onClose && (
          <button
            ...
            title="Fechar e continuar no sistema"
            id="btn-close-login"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-3.5 mb-5 space-y-2">
          <div className="flex items-center justify-between text-blue-900 font-bold text-xs">
            ...
            <span className="text-[10px] text-blue-600/80 font-normal">1-clique</span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={() => handleQuickLogin("admin@otica.com", "admin")}
              className="px-3 py-2.5 bg-white hover:bg-blue-100/70 border border-blue-200 hover:border-blue-300 text-blue-900 font-semibold text-xs rounded-xl shadow-2xs flex flex-col items-start transition-all active:scale-95 text-left cursor-pointer group"
              id="btn-quick-admin"
            >
              <span className="flex items-center gap-1 font-bold text-[11px] text-blue-700 group-hover:text-blue-900">
                <Shield className="w-3 h-3 text-blue-600" /> Administrador
              </span>
              <span className="text-[10px] text-slate-500 truncate w-full mt-0.5">Dra. Helena Martins</span>
            </button>
            ...
            <button
              ...
              className="px-3 py-2.5 bg-white hover:bg-blue-100/70 border border-blue-200 hover:border-blue-300 text-blue-900 font-semibold text-xs rounded-xl shadow-2xs flex flex-col items-start transition-all active:scale-95 text-left cursor-pointer group"
              id="btn-quick-atendente"
            >
              <span className="flex items-center gap-1 font-bold text-[11px] text-slate-700 group-hover:text-blue-900">
                <User className="w-3 h-3 text-slate-500 group-hover:text-blue-600" /> Atendente
              </span>
              <span className="text-[10px] text-slate-500 truncate w-full mt-0.5">Lucas Andrade</span>
            </button>
          </div>
        </div>
        ...
        {onClose && (
          <div className="mt-4 pt-3 border-t border-slate-100 text-center">
            <button
              type="button"
              onClick={onClose}
              className="text-xs font-semibold text-slate-500 hover:text-slate-800 hover:underline py-1 px-3 rounded-lg transition-colors cursor-pointer"
              id="btn-cancel-modal"
            >
              Voltar e continuar no sistema
            </button>
          </div>
        )}
      </div>
